...
ggplot(dt, aes(period, power, group=id)) + geom_line(alpha=.3) +
   scale_x_log10() + facet_wrap(~ taxon, nrow=2 )  
```
...
labs[, id := label]
setkey(labs,id)
labs <- unique(dt)[,.(id,taxon_id)][labs]
ddata$labels <- labs

ggplot(segment(ddata)) + 
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend)) + 
  geom_text(data = ddata$labels, 
              aes(x = x, y = y, label = label, colour=taxon_id), size = 3, vjust = 0.5, hjust=0)+ 
  coord_flip() +
...
* Curate original data by ensuring annotations are not mislabelled (see annotation 244 ans 274)
